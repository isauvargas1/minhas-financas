...
const seedTestData = async (): Promise<{
  ...
  const workspaceId = `workspace-close-invoice-test-${suffix}`;
  const uid = `user-close-invoice-test-${suffix}`;
  const cardId = `card-close-invoice-test-${suffix}`;
...
const createPurchase = async (
  ...
  const payload = createCreditCardPurchasePayloadSchema.parse({
    workspaceId,
    cardId,
    idempotencyKey: `manual-create-before-close-${Date.now()}`,
    correlationId: "manual-test-close-create-purchase",

    description: "Compra para teste de fechamento de fatura",
    categorySnapshot: {
      label: "Teste",
...
const closeInvoice = async (
  ...
  const payload = closeCreditCardInvoicePayloadSchema.parse({
    ...
    invoiceId,
    idempotencyKey,
    correlationId: "manual-test-close-invoice",
    closedAt: "2026-05-10",
  });
...
const assertCloseResult = (
  firstResult: CloseCreditCardInvoiceResult,
  secondResult: CloseCreditCardInvoiceResult
): void => {
  if (!firstResult.success) {
    throw new Error("A primeira execução do fechamento não retornou sucesso.");
  }
  ...
  if (firstResult.invoice.status !== "closed") {
    throw new Error("A fatura deveria estar closed.");
  }

  if (firstResult.invoice.totalAmount !== 400) {
    throw new Error("O total da fatura deveria ser 400.");
  }

  if (firstResult.invoice.paidAmount !== 0) {
    throw new Error("O valor pago da fatura deveria ser 0.");
  }

  if (firstResult.invoice.remainingAmount !== 400) {
    throw new Error("O saldo restante da fatura deveria ser 400.");
  }

  if (firstResult.invoice.itemsCount !== 1) {
    throw new Error("A primeira fatura deveria ter 1 item.");
  }
};
...
const inspectCreatedDocuments = async (
  workspaceId: string,
  invoiceId: string,
  result: CloseCreditCardInvoiceResult
): Promise<void> => {
  const db = admin.firestore();
  const workspaceRef = db.doc(`workspaces/${workspaceId}`);
  ...
  if (!eventSnap.exists) {
    throw new Error("Evento financeiro do fechamento não foi criado.");
  }

  if (installmentsSnap.size !== 1) {
    throw new Error(
      `Esperado 1 parcela na fatura, encontrado ${installmentsSnap.size}.`
    );
  }
  ...
  if (invoiceData?.status !== "closed") {
    throw new Error("A fatura persistida deveria estar closed.");
  }
  ...
  if (invoiceData?.itemsCount !== 1) {
    throw new Error("A fatura persistida deveria ter itemsCount 1.");
  }

  if (invoiceViewData?.status !== "closed") {
    throw new Error("A projeção da fatura deveria estar closed.");
  }
  ...
  if (eventData?.eventType !== "invoice_closed") {
    throw new Error("O evento financeiro deveria ser invoice_closed.");
  }
};
...
const run = async (): Promise<void> => {
  ...
  const closeIdempotencyKey = `manual-close-invoice-${Date.now()}`;

  const firstCloseResult = await closeInvoice(
    workspaceId,
    uid,
    cardId,
    invoiceId,
  ...
  const secondCloseResult = await closeInvoice(
    workspaceId,
    uid,
    cardId,
    invoiceId,
  ...
  assertCloseResult(firstCloseResult, secondCloseResult);
  ...
  console.log("Teste manual closeCreditCardInvoice concluído com sucesso.");
  console.log(JSON.stringify(firstCloseResult, null, 2));
};

run()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error("Falha no teste manual closeCreditCardInvoice.");
    console.error(error);
    process.exit(1);
  });