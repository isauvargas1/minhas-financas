...
const CreditCard3D: React.FC<CreditCard3DProps> = ({ card, mode, limits, onClick }) => {
    ...
    const textColorClass = card.visual.textColor === 'white' ? 'text-white' : 'text-gray-900';
    const borderColorClass = card.visual.textColor === 'white' ? 'border-white/20' : 'border-gray-900/20';
            ...
            >
                ...
                >
                    ...
                    <div className="flex justify-between items-start z-10">
                        {card.visual.showName && <span className={`font-medium tracking-wide ${textColorClass}`}>{card.name}</span>}
                        {card.visual.showLogo && <div className={`opacity-70 ${textColorClass}`}><WifiIcon className="w-6 h-6 rotate-90" /></div>}
                    </div>
                    ...
                    <div className="z-10">
                        ...
                        <div className="flex justify-between items-end">
                            <div className={`text-xs ${textColorClass} opacity-80 uppercase`}>
                                {card.visual.showName ? 'Usuario' : ''}
                            </div>
                            {card.visual.showBrand && <span className={`font-bold italic text-lg ${textColorClass}`}>{card.brand}</span>}
                        </div>
                    </div>
                </div>
