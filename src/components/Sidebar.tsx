...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    const menuItems = [
        ...
        ...(isPJ
            ? [
                  {
                      icon: <BriefcaseIcon />,
                      text: 'Clientes & Recebíveis',
                      id: 'clients_receivables',
                  },
                  {
                      icon: <HandshakeIcon />,
                      text: 'Empréstimos Corp.',
                      id: 'loans',
                  },
              ]
            : [
    ...
    return (
        <aside
            className={`fixed lg:relative z-30 h-screen bg-surface border-r border-border shadow-lg flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${
                isExpanded ? 'w-[var(--sidebar-width)]' : 'w-0 lg:w-16'
            }`}
        ...
        >
            <div className="p-4 flex items-center justify-center border-b border-border min-h-[72px]">
                ...
                <span
                    className={`ml-3 font-bold text-primary text-xl whitespace-nowrap transition-opacity duration-200 ${
                    ...
                    {menuItems.map((item) => (
                        <li key={item.id}>
                            <a
                                ...
                                className={`flex items-center py-3 px-5 border-l-4 hover:bg-background transition-colors ${
                                    currentView === item.id
            ...
            <div className="p-4 border-t border-border">
                <button
                    ...
                    className="w-full flex items-center py-2 px-4 text-muted rounded-card hover:bg-background transition-colors"
