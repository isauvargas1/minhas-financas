...
const RecurringExpenseCard: React.FC<RecurringExpenseCardProps> = ({ expense, onClick }) => {
    ...
    return (
        <MotionDiv
            ...
            className={`bg-white dark:bg-dark-100 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 cursor-pointer group flex flex-col relative overflow-hidden h-full ${expense.status !== 'ativo' ? 'opacity-75 grayscale-[0.3]' : ''}`}
        >
            {/* Top Color Bar */}
            ...
            <div className="p-5 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                    ...
                    >
                        {expense.emojiOpcional ? <span className="text-xl">{expense.emojiOpcional}</span> : <DynamicIcon name={expense.icone} size={24} />}
                    </div>
                ...
                <div className="mb-4">
                    <h3 className="font-bold text-gray-800 dark:text-white text-lg leading-tight mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                        {expense.nome || 'Sem Nome'}
                    </h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 capitalize bg-gray-100 dark:bg-dark-200 inline-block px-2 py-0.5 rounded">
                ...
                <div className="flex gap-2 mb-4 mt-auto">
                    {expense.cartaoIdOpcional && (
                        <div className="flex items-center justify-center w-6 h-6 rounded bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-300" title="Vinculado a Cartão">
                            <CreditCardIcon className="w-3 h-3" />
                        </div>
                    )}
                    {expense.splitGroupIdOpcional && (
                        <div className="flex items-center justify-center w-6 h-6 rounded bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-300" title="Grupo Dividido">
                            <UsersIcon className="w-3 h-3" />
                        </div>
                    )}
                    {expense.gerarDespesaAutomaticamente && (
                        <div className="flex items-center justify-center w-6 h-6 rounded bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-300" title="Geração Automática">
                ...
                <div className="border-t border-gray-100 dark:border-gray-800 pt-3 flex justify-between items-end">
                    <div>
                        <span className="text-[10px] text-gray-400 uppercase font-bold block mb-0.5">Próxima</span>
