...
type Editor = 'account' | 'asset' | 'contribution' | 'redemption' | 'settle' | 'link' | 'reverse' | null;
...
export const InvestmentsPortfolioView: React.FC<Props> = ({ workspaceId, canManage, goals, onBack }) => {
  ...
  const submitEntity = (event: React.FormEvent<HTMLFormElement>, kind: 'account' | 'asset') => {
    ...
    mutation.mutate({ name: kind === 'account' ? 'saveInvestmentAccount' : 'saveInvestmentAsset', payload: kind === 'account'
      ...
      : { assetId: selected?.id, name: form.get('name'), symbol: form.get('symbol') || undefined, assetType: form.get('assetType') } });
  };
  ...
  return <section className="mx-auto max-w-7xl space-y-5" aria-labelledby="investments-title">
    ...
    <div role="tablist" aria-label="Seções de investimentos" className="flex gap-2 overflow-x-auto border-b border-outline/20">
      {tabs.map(([id, label]) => <button key={id} role="tab" aria-selected={tab === id} onClick={() => setTab(id)} className={`whitespace-nowrap px-4 py-3 ${tab === id ? 'border-b-2 border-primary font-semibold text-primary' : ''}`}>{label}</button>)}
    </div>

    {tab === 'summary' && <div><State loading={summary.isLoading} error={summary.isError} />{summary.data && <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    ...
    {tab === 'accounts' && <div className="space-y-4">
    ...
    {tab === 'movements' && <div className="space-y-4">
      ...
      <div className="space-y-3">{movements.data?.items.map((item) => <article key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4"><div><h3 className="font-semibold">{item.description}</h3><p className="text-sm text-on-surface-variant">{item.operation === 'contribution' ? 'Aporte' : item.operation === 'redemption' ? 'Resgate' : item.operation === 'reversal' ? 'Reversão' : 'Vínculo com meta'} · {item.status === 'pending' ? 'Pendente' : 'Liquidada'} · {date(item.occurredAt)}</p></div><div className="text-right"><p className="font-semibold">{money(item.principalCents)}</p>{item.operation === 'redemption' && item.status === 'pending' && <button onClick={() => { setSelected(item); setEditor('settle'); }} className="text-sm text-primary">Liquidar</button>}{canManage && item.status === 'settled' && !item.reversedByMovementId && ['contribution', 'redemption'].includes(item.operation) && <button onClick={() => { setSelected(item); setEditor('reverse'); }} className="ml-3 text-sm text-red-700">Reverter</button>}</div></article>)}</div>
    ...
    <Modal title={editor === 'account' ? 'Conta de investimento' : editor === 'asset' ? 'Ativo de investimento' : editor === 'contribution' ? 'Novo aporte' : editor === 'redemption' ? 'Solicitar resgate' : editor === 'settle' ? 'Liquidar resgate' : editor === 'link' ? 'Vincular posição a uma meta' : 'Reverter movimentação'} open={editor !== null} onClose={() => { setEditor(null); setSelected(null); }}>
      ...
      {editor === 'asset' && <form onSubmit={(e) => submitEntity(e, 'asset')} className="grid gap-4"><Field label="Nome do ativo" name="name" required defaultValue={(selected as InvestmentAsset | null)?.name} /><Field label="Código do ativo (opcional)" name="symbol" defaultValue={(selected as InvestmentAsset | null)?.symbol} /><label className="grid gap-1 text-sm font-medium">Tipo<select name="assetType" className="rounded-lg border px-3 py-2" defaultValue={(selected as InvestmentAsset | null)?.assetType ?? 'fixed_income'}><option value="fixed_income">Renda fixa</option><option value="fund">Fundo</option><option value="stock">Ação</option><option value="etf">ETF</option><option value="crypto">Criptoativo</option><option value="other">Outro</option></select></label><button disabled={mutation.isPending} className="rounded-lg bg-primary px-4 py-2 text-on-primary">Salvar ativo</button></form>}
