...
| INV-P2-044 | FIXED | Duas causas distintas: no E2E, o servidor de desenvolvimento entregava o grafo de 12.000 módulos sem empacotar e o renderer do Chromium morria com "Page crashed" antes de qualquer asserção; a suíte de integração falhava por ordenação de leitura/escrita em transação. | O E2E passa a rodar contra o **build de produção** servido por `vite preview` (maior fidelidade, e o bundle principal caiu de ~10 MB para 827 kB com a divisão por vendor). O build acontece antes do Playwright, para não competir com a JVM dos emuladores. | Suíte E2E completa: 24 de 24, sem flake e sem retry. Integração: 92 de 92. | fix(investments) |
