...
| NEW-06 | P1 | **Meta acima de 2.000 aportes virava beco sem saída.** O teto de varredura em transação era correto — uma transação do Firestore não soma o histórico inteiro —, mas acima dele o aporte era recusado, o arquivamento era recusado, o gatilho lançava, e a "reconstrução paginada" citada na mensagem de erro tinha exatamente o mesmo teto e não era paginada. Não havia caminho de volta. Encontrado pela revisão de escala e custo do Firestore ao cobrar o item "correção não pode depender de rotina inexistente". | FIXED — `rebuildGoalProgress` soma por páginas com cursor fora da transação e publica valor absoluto (teto real de 100.000 aportes, erro nomeado acima disso); o gatilho mantém o progresso por delta acima do teto, como já faz com a projeção mensal de caixa no mesmo arquivo; arquivar deixou de depender do histórico. Recusa permanece só onde é fisicamente correta: refazer o vínculo de mais de 2.000 aportes ultrapassaria o limite de escritas de uma transação. Teste semeia 2.001 aportes e exige aporte aceito, reconstrução idempotente e arquivamento. | fix(goals) |
| NEW-07 | P2 | **Pacote de 3 MB de ícones no primeiro paint, e falha de recurso disfarçada de flakiness.** `AppIcon` pedia `tabler` junto do pacote do tema em toda renderização, para usá-lo só como recurso quando um nome não existe em `lucide`. Somado à JVM do emulador sem teto de heap, o contêiner ficava com menos de 900 MB livres durante o E2E e o renderer do Chromium morria com "Target crashed" em pontos diferentes a cada execução. | FIXED — `tabler` só é pedido quando o pacote do tema já chegou e realmente não tem o ícone; `useIconPacks` filtra entrada vazia (`''.split(',')` devolve `['']`); o emulador do Firestore recebe teto de heap no E2E. Folga mínima medida durante a suíte passou de 853 MB para 1.994 MB, e a suíte completa passou 29/29 em execuções consecutivas. | perf(ui) |
